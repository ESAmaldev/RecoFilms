...
// Serve static built SPA if present (expects client build in ../recofilms/dist)
// __dirname isn’t defined in ESM; compute it from import.meta.url
import { fileURLToPath } from 'url';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const possibleStaticDirs = [
  path.join(__dirname, '..', 'recofilms', 'dist'),
  path.join(__dirname, '..', 'dist'),
  path.join(__dirname, '..', '..', 'dist')
];
let staticDir = possibleStaticDirs.find((d) => fs.existsSync(d));
if (staticDir) {
  ...
  app.get('/', (req, res) => {
    res.send('Proxy running. Client build not found. Build the client in the project root with `npm run build`.');
  });
}
